...
class PySREmu:
    ...
    def equation_(self, ns, hub, hireionz, x1, x2):
        """
        x1: normalized k
        ...
        return (
            ...
            + 0.0
            + self.equation3(hireionz, x1, x2)
            - self.equation3(self.hireionz_fid, x1, x2)
        ) + np.mean(
            [
                self.equation1(self.ns_fid, x1, x2),
                self.equation2(self.hub_fid, x1, x2),
    ...
    def equation1(self, ns, x1, x2):
        ...
        only vary ns
        """
        return  (np.sin(1.3866498 - np.exp(ns + (x1**0.29783842))) - x2) + (x1 * ns)
    ...
    def equation2(self, hub, x1, x2):
        ...
        only vary hub
        """
        return (((x2)**2 - x1) * (1.1136419 - x1)) * -4.332934

    
    def equation3(self, hireionz, x1, x2):
        """
        only vary hirieonz
        """
        return (x2 * 4.2581806) * ((x1**(hireionz**x1)) - x2)
    ...
    def predict(self, X):
        ...
        for _x in X:
            # _x is (4, )
            ns, hub, hireionz, x1, x2 = _x
            this_y_pred = self.equation_(ns, hub, hireionz, x1, x2)
